...
const setupScript = `
-- =================================================================
-- السكربت الشامل والنهائي (الإصدار 34.0) - مزامنة الملفات
-- =================================================================
...
DROP FUNCTION IF EXISTS public.check_if_mobile_exists(mobile text) CASCADE;
...
CREATE OR REPLACE FUNCTION public.check_if_mobile_exists(mobile text)
RETURNS boolean AS $$
BEGIN
    -- This function runs with SECURITY DEFINER to bypass RLS and check for an existing mobile number.
    -- It is callable by anonymous users to prevent duplicate signups.
    RETURN EXISTS (SELECT 1 FROM public.profiles WHERE mobile_number = mobile);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION public.check_if_mobile_exists(mobile text) TO anon, authenticated;
...
CREATE TABLE public.case_documents (id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY, user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, case_id text NOT NULL REFERENCES public.cases(id) ON DELETE CASCADE, file_name text NOT NULL, file_path text NOT NULL, mime_type text NOT NULL, created_at timestamptz DEFAULT now());
...
BEGIN
  ...
  -- It now uses the mobile number directly from the metadata provided during signup.
...
-- 8. STORAGE SETUP
-- Create a private bucket for case documents.
INSERT INTO storage.buckets (id, name, public)
VALUES ('case-documents', 'case-documents', false)
ON CONFLICT (id) DO NOTHING;

-- Storage RLS policies for 'case-documents' bucket
DROP POLICY IF EXISTS "Allow select for own documents" ON storage.objects;
CREATE POLICY "Allow select for own documents" ON storage.objects
FOR SELECT USING (bucket_id = 'case-documents' AND auth.uid() = owner);

DROP POLICY IF EXISTS "Allow insert for own documents" ON storage.objects;
CREATE POLICY "Allow insert for own documents" ON storage.objects
FOR INSERT WITH CHECK (bucket_id = 'case-documents' AND auth.uid() = owner);

DROP POLICY IF EXISTS "Allow update for own documents" ON storage.objects;
CREATE POLICY "Allow update for own documents" ON storage.objects
FOR UPDATE USING (bucket_id = 'case-documents' AND auth.uid() = owner);

DROP POLICY IF EXISTS "Allow delete for own documents" ON storage.objects;
CREATE POLICY "Allow delete for own documents" ON storage.objects
FOR DELETE USING (bucket_id = 'case-documents' AND auth.uid() = owner);
...
-- =================================================================
-- 10. إنشاء حساب المدير تلقائياً وتصحيحه
-- =================================================================
        ...
        <div className="flex items-center justify-center min-h-screen bg-gray-100 p-4" dir="rtl">
            <div className="w-full max-w-4xl p-8 space-y-6 bg-white rounded-lg shadow-md">
                <div className="text-center">
                    <h1 className="text-3xl font-bold text-gray-800">إعداد قاعدة البيانات</h1>
                     <p className="mt-2 text-gray-600">
                        يبدو أن قاعدة البيانات غير مهيأة أو تحتاج لإعادة ضبط. يرجى اتباع الخطوات التالية لإعداد التطبيق بشكل كامل.
                    </p>
                </div>
                ...
                <div className="space-y-4 animate-fade-in">
                    ...
                    <ScriptDisplay script={setupScript} />
                     <ol className="list-decimal list-inside space-y-2 text-gray-700">
                        ...
                        <li>
                            <strong className="text-red-600">تأكد من تعديل رقم هاتف وكلمة مرور المدير</strong> في نهاية السكربت (القسم 10).
                        </li>
