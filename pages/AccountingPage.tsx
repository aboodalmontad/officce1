...
const EntriesTab: React.FC = () => {
    const { accountingEntries, setAccountingEntries, clients, cases, deleteAccountingEntry } = useData();
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const entryData = { 
            ...
            clientName: selectedClient?.name || 'مصاريف عامة', 
...
const InvoicesTab: React.FC<InvoicesTabProps> = ({ initialInvoiceData, clearInitialInvoiceData }) => {
    const { invoices, setInvoices, clients, cases, deleteInvoice } = useData();
    ...
    const handleOpenModal = (invoice?: Invoice, initialIds?: { clientId: string; caseId?: string }) => {
        ...
        const caseItem = cases.find(c => c.id === initialIds?.caseId);
...
const ReportsTab: React.FC = () => {
    const { clients, cases, accountingEntries } = useData();
    ...
    const generateCaseStatusReport = () => { const allCases = cases; const filteredCases = filters.clientId === 'all' ? allCases : allCases.filter(c => c.clientId === filters.clientId); const caseCounts = filteredCases.reduce((acc, caseItem) => { acc[caseItem.status] = (acc[caseItem.status] || 0) + 1; return acc; }, {} as Record<Case['status'], number>); const statusMap: Record<Case['status'], string> = { active: 'نشطة', closed: 'مغلقة', on_hold: 'معلقة' }; setReportData({ type: 'cases', cases: filteredCases, pieData: Object.entries(caseCounts).map(([status, value]) => ({ name: statusMap[status as Case['status']], value })), title: `تقرير حالة القضايا ${filters.clientId !== 'all' ? `للموكل: ${clients.find(c => c.id === filters.clientId)?.name}` : ''}` }); };
    const generateClientActivityReport = () => { if (filters.clientId === 'all') { setReportData(null); alert('يرجى اختيار موكل لعرض تقرير النشاط.'); return; } const client = clients.find(c => c.id === filters.clientId); if (!client) return; const clientEntries = accountingEntries.filter(e => e.clientId === client.id); const totals = clientEntries.reduce((acc, entry) => { if (entry.type === 'income') acc.income += entry.amount; else acc.expense += entry.amount; return acc; }, { income: 0, expense: 0 }); setReportData({ type: 'clients', client, cases: cases.filter(c => c.clientId === client.id), entries: clientEntries.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()), totals: { ...totals, balance: totals.income - totals.expense }, title: `تقرير نشاط الموكل: ${client.name}` }); };
    const generateAnalyticsReport = () => { const topClientsByCases = [...clients].map(c => ({ name: c.name, value: cases.filter(cs => cs.clientId === c.id).length })).filter(c => c.value > 0).sort((a, b) => b.value - a.value).slice(0, 7); const incomeByClient = accountingEntries.filter(e => e.type === 'income' && e.clientId).reduce((acc, entry) => { acc[entry.clientId] = (acc[entry.clientId] || 0) + entry.amount; return acc; }, {} as Record<string, number>); const topClientsByIncomeData = Object.entries(incomeByClient).map(([clientId, income]): { name: string; value: number } => ({ name: clients.find(c => c.id === clientId)?.name || 'غير معروف', value: income as number })).sort((a, b) => b.value - a.value); const topClientsByIncome = topClientsByIncomeData.slice(0, 5); if (topClientsByIncomeData.length > 5) { const othersIncome = topClientsByIncomeData.slice(5).reduce((acc, curr) => acc + curr.value, 0); topClientsByIncome.push({ name: 'آخرون', value: othersIncome }); } const allCases = cases; const incomeByCase = accountingEntries.filter(e => e.type === 'income' && e.caseId).reduce((acc, entry) => { acc[entry.caseId] = (acc[entry.caseId] || 0) + entry.amount; return acc; }, {} as Record<string, number>); const topCasesByIncome = Object.entries(incomeByCase).map(([caseId, income]) => { const caseInfo = allCases.find(c => c.id === caseId); const name = caseInfo ? `${caseInfo.clientName} - ${caseInfo.subject}` : 'قضية محذوفة'; return { name: name.length > 40 ? name.slice(0, 37) + '...' : name, value: income as number }; }).sort((a, b) => b.value - a.value).slice(0, 10); setReportData({ type: 'analytics', data: { topClientsByCases, topClientsByIncome, topCasesByIncome, }, title: 'تحليلات شاملة لأداء المكتب' }); };
