...
const EntriesTab: React.FC = () => {
    ...
    // Relaxed type to 'any' to handle string dates from input fields easily
    ...
    const handleOpenModal = (entry?: AccountingEntry) => {
        // Using toInputDateString for safe conversion without type conflicts
        setFormData(entry ? { ...entry, date: toInputDateString(entry.date) } : { type: 'income', date: toInputDateString(new Date()) });
        setModal({ isOpen: true, data: entry });
    };
    const handleCloseModal = () => setModal({ isOpen: false });
    const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        const { name, value } = e.target;
        setFormData((prev: any) => ({ ...prev, [name]: name === 'amount' ? parseFloat(value) : value }));
    };
...
const InvoicesTab: React.FC<InvoicesTabProps> = ({ initialInvoiceData, clearInitialInvoiceData }) => {
    ...
    // Relaxed type to 'any' to handle string dates and nested items easily
    const [formData, setFormData] = React.useState<any>({ items: [] });
    const [searchQuery, setSearchQuery] = React.useState('');
    ...
    const handleOpenModal = (invoice?: Invoice, initialIds?: { clientId: string; caseId?: string }) => {
        ...
        const newFormData = invoice ? { ...invoice, issueDate: toInputDateString(invoice.issueDate), dueDate: toInputDateString(invoice.dueDate) } : { id: `INV-${Date.now()}`, issueDate: toInputDateString(today), dueDate: toInputDateString(dueDate), clientId: client?.id || '', caseId: caseItem?.id || '', items: [{ id: `item-${Date.now()}`, description: '', amount: 0, updated_at: new Date() }], taxRate: 0, discount: 0, status: 'draft' as 'draft' };
        setFormData(newFormData);
        setModal({ isOpen: true, data: invoice });
    };
    const handleCloseModal = () => setModal({ isOpen: false });
    const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => { const { name, value } = e.target; const numValue = (name === 'taxRate' || name === 'discount') ? parseFloat(value) : value; setFormData((prev: any) => ({ ...prev, [name]: numValue })); };
    const handleItemChange = (index: number, field: keyof InvoiceItem, value: string | number) => { const newItems = [...formData.items]; const updatedItem = { ...newItems[index], updated_at: new Date() }; (updatedItem as any)[field] = field === 'amount' ? parseFloat(value as string) || 0 : value; newItems[index] = updatedItem; setFormData((prev: any) => ({ ...prev, items: newItems })); };
    const handleAddItem = () => setFormData((prev: any) => ({ ...prev, items: [...prev.items, { id: `item-${Date.now()}`, description: '', amount: 0, updated_at: new Date() }] }));
    const handleRemoveItem = (index: number) => setFormData((prev: any) => ({ ...prev, items: prev.items.filter((_: any, i: number) => i !== index) }));
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        ...
        const finalInvoice: Invoice = { id: formData.id!, clientId: client.id, clientName: client.name, caseId: caseItem?.id, caseSubject: caseItem?.subject, issueDate: parsedIssueDate, dueDate: parsedDueDate, items: formData.items.filter((item: any) => item.description.trim() !== '' && item.amount > 0), taxRate: formData.taxRate || 0, discount: formData.discount || 0, status: formData.status!, notes: formData.notes, updated_at: new Date() };
    ...
    const subtotal = formData.items?.reduce((sum: number, item: any) => sum + (item.amount || 0), 0) || 0;
